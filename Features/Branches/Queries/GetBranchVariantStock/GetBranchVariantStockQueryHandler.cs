...
namespace Inventory_Management_System.Features.Branches.Queries.GetBranchVariantStock
{
    ...
    {
        public async Task<Result> Handle(GetBranchVariantStockQuery request, CancellationToken cancellationToken)
        {
            if (request.BranchId <= 0 || request.ProductVariantId <= 0)
                return new Result { IsSuccess = false, StatusCode = 400, Status = "Error", Message = "A valid branch id and product variant id are required." };
            ...
            try
            {
                ...
                var zero = new BranchVariantStockResponse(
                    branch.Id, branch.Name, variant.Id, variant.SKU, variant.ProductName, variant.IsSerialized, 0);
            ...
            {
                _logger.LogError(ex, "Error retrieving stock for branch {BranchId} variant {VariantId}", request.BranchId, request.ProductVariantId);
